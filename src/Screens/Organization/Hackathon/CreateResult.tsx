...
const Card: FC<cardProps> = ({title, image, score, rating}) => {
  const {theme} = useStateValue()[0];
  const [LogoImage, setLogoImage] = useState(true);
  const [selected, setselected] = useState(false);

  return (
    <TouchableOpacity activeOpacity={0.5} onPress={() => setselected(true)}>
      <View
        style={[
        ...
        <View style={{flexDirection: 'row'}}>
          <View style={[styles.selectionContainer, styles.center]}>
            <CheckBox
              size={20}
              onPress={isChecked =>
                setselected(typeof isChecked !== 'undefined' && isChecked)
              }
